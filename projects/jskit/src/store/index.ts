...
export function derived(stores, fn, initial_value)
{
	...
	return readable(initial_value, (set, update) =>
  {
		...
		return function stop()
    {
			...
			cleanup();
      // We need to set this to false because callbacks can still happen
      // despite having unsubscribed:
      // Callbacks might already be placed in the queue which doesn't know it
      // should no longer invoke this derived store.
			started = false;
		};
	});
}
...
/**
 * Takes a store and returns a new one derived from the old one that is
 * readable.
 *

...
import { log } from "./log";
import { AnyConstructor, code, FcodeCore } from "./fcode";
import { Queue } from "queue-typescript";
import { AlreadyProcessedErr, InpErr, NotFoundErr } from "./err";
import { catchError, map, Observable, ReplaySubject } from "rxjs";
import { AlertService, AlertLevel } from "ngx-kit";
...
export class ClientBus
{
  ...
  private serverConn$: WebSocketSubject<Rawmsg>;
  ...
  ): void
  {
    ...
    this.serverConn$ = webSocket(
      "ws://"
      + environment.cpasbUrl.host
      + ":"
      + environment.cpasbUrl.port
      + "/rx"
    );

    // this causes strange undefined err, but everything works fine, we don't
    // know why, for now
    this.serverConn$.subscribe({
  ...
  ): void
  {
    ...
    if (opts.isNetSendSkipped !== true)
    {
      ...
      this.serverConn$.next(serializedMsg);
    }
